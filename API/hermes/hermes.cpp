...
jsi::Value HermesRuntimeImpl::evaluateJavaScript(
  ...
  if (isMainBundle) {
    ...
    evaluateJavaScript(
        ...
        std::make_unique<jsi::StringBuffer>(std::string("(()=>{function asyncGeneratorStep(gen,resolve,reject,_next,_throw,key,arg){try{var info=gen[key](arg);var value=info.value;}catch(error){reject(error);return}if(info.done){resolve(value);}else {Promise.resolve(value).then(_next,_throw);}}function _asyncToGenerator(fn){return function(){var self=this,args=arguments;return new Promise(function(resolve,reject){var gen=fn.apply(self,args);function _next(value){asyncGeneratorStep(gen,resolve,reject,_next,_throw,'next',value);}function _throw(err){asyncGeneratorStep(gen,resolve,reject,_next,_throw,'throw',err);}_next(undefined);})}}_asyncToGenerator(function*(){var{externalStorageDirectory,requestPermissions,download,checkPermissions}=nativeModuleProxy.AliucordNative;function downloadAliucord(){return _downloadAliucord.apply(this,arguments)}function _downloadAliucord(){_downloadAliucord=_asyncToGenerator(function*(){var bundlePath=externalStorageDirectory+'/AliucordRN/Aliucord.js.bundle';if(!AliuFS.exists(bundlePath)){try{yield download('https://raw.githubusercontent.com/Aliucord/AliucordRN/builds/Aliucord.js.bundle',bundlePath);}catch(error){alert('Failed to download Aliucord.js.bundle');throw error}}(globalThis._globals\?\?={}).aliucord=AliuHermes.run(bundlePath);});return _downloadAliucord.apply(this,arguments)}try{var granted=yield checkPermissions();if(!granted){var dialogResult=yield new Promise((res,rej)=>{window.nativeModuleProxy.DialogManagerAndroid.showAlert({title:'Storage Permissions',message:'Aliucord needs access to your storage to load plugins and themes.',cancellable:true,buttonPositive:'Open'},rej,action=>{if(action===constants.buttonClicked)res(true);else res(false);});});if(dialogResult){alert('Access to your storage is required for aliucord to load.');return}var permissionsGranted=yield requestPermissions();if(!permissionsGranted){alert('Access to your storage is required for aliucord to load.');return}}yield downloadAliucord();}catch(error){alert('Something went wrong :(\\nCheck logs');console.error(error.stack);}})();})();")),
        "bootstrap"
    );
  }
